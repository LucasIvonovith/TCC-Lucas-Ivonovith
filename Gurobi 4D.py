...
def Gurobi4D(a1,a2,a3,a4,c1,c2,c3,c4):
  ...
  x4 = elipse_model.addVar(lb=0, ub=GRB.INFINITY, vtype=GRB.INTEGER, name="x4")#criando variavel x2
  elipse_model.update()

  elipse_model.setObjective(c1 * x1 + c2 * x2 +c3*x3+c4*x4, GRB.MAXIMIZE)#criando objetivo(maximizar k)

  elipse_model.addQConstr((x1**2)/(a1**2) + (x2**2)/(a2**2) +(x3**2)/(a3**2)+(x4**2)/(a4**2) <= 1, name="elipse")#criando restrição(dentro da elipse)
  ...
  if elipse_model.Status == GRB.OPTIMAL:
    return (np.array([x1.X, x2.X,x3.X,x4.X]))
  else:
    return None


columns=['A1','A2','A3','A4','C1','C2','C3','C4','Tempo','K max','Ponto','Dentro Elips']
ncasos=1000
seed=74
pont=[]
res=[]
...
for j in range (ncasos):
    print(j)
    a=np.array([ai[j], aii[j],aiii[j],aiiii[j]])
    c=np.array([ci[j], cii[j],ciii[j],ciiii[j]])
    init = time.perf_counter()
    (ponto)=Gurobi4D(a[0],a[1],a[2],a[3],c[0],c[1],c[2],c[3])
    endt = time.perf_counter()
    tim.append((endt-init))
    res.append(ponto@c)
    pont.append(ponto)

    if ((ponto[3]**2/a[3]**2)+ (ponto[2]**2 / a[2]**2)+ (ponto[1]**2 / a[1]**2)+ (ponto[0]**2 / a[0]**2))<=1:
      dentro.append(1)
    else:
      dentro.append(0)

    df = pd.DataFrame(list(zip(ai,aii,aiii,aiiii,ci,cii,ciii,ciiii,tim,res,pont,dentro)), columns=columns)
    df.to_excel(r"C:\Users\lucas_mzj9txp\Desktop\Gurobi4D.xlsx")