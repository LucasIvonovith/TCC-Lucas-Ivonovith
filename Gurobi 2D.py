...
def Gurobi2D(a1,a2,c1,c2):
  ...
  elipse_model.setObjective(c1 * x1 + c2 * x2, GRB.MAXIMIZE)#criando objetivo(maximizar k)

  elipse_model.addQConstr((x1**2)/(a1**2) + (x2**2)/(a2**2) <= 1, name="elipse")#criando restrição(dentro da elipse)

  elipse_model.update()
  elipse_model.optimize()


  if elipse_model.SolCount > 0:
        return np.array([x1.X, x2.X])
  else:
        return None


columns=['A1','A2','C1','C2','Tempo','K max','Ponto','Dentro Elips']
ncasos=1000
seed=74
pont=[]
res=[]
...
for j in range (ncasos):
    ...
    (ponto)=Gurobi2D(a[0],a[1],c[0],c[1])
    ...
    df = pd.DataFrame(list(zip(ai,aii,ci,cii,tim,res,pont,dentro)), columns=columns)
    df.to_excel(r"C:\Users\lucas_mzj9txp\Desktop\Gurobi2D.xlsx")