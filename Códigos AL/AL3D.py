...
def relaxado (a1,a2,i3,c1,c2,c,kmax,pmaior,npontos):
...
def AL3D(a,c):
    ...
    for i3 in range (floor(a[2])+1):
                ...
                a_2=a[1]*sqrt(raiz1)
                if a_1!=0:
                    (kmax,pmaior,npontos)=relaxado(a_1,a_2,i3,c[0],c[1],c,kmax,pmaior,npontos)
                else:
                    pt=np.array([0,0,i3])
                    if pt@c>kmax:
                        pmaior=pt
...
for j in range (ncasos):
    ...
    df.to_excel(r"C:\Users\lucas_mzj9txp\Desktop\AL3D.xlsx")