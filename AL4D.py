...
def relaxado (a1,a2,i3,i4,c1,c2,c,kmax,pmaior,npontos):
...
def AL4D(a,c):
    kmax=0
    npontos=0
    pmaior=np.array([0,0,0,0])

    for i4 in range (floor(a[3])+1):
            raiz2= max(0.0,1 -(i4**2/a[3]**2))
            a_3=a[2]*sqrt(raiz2)
            for i3 in range (floor(a_3)+1):
                raiz1= max(0.0,1-(i3**2/a[2]**2) -(i4**2/a[3]**2))

                a_1=a[0]*sqrt(raiz1)
                a_2=a[1]*sqrt(raiz1)
                if a_1!=0:
                    (kmax,pmaior,npontos)=relaxado(a_1,a_2,i3,i4,c[0],c[1],c,kmax,pmaior,npontos)
                else:
                    pt=np.array([0,0,i3,i4])
                    if pt@c>kmax:
                        pmaior=pt
                        kmax=pt@c
...
columns=['A1','A2','A3','A4','C1','C2','C3','C4','Tempo','K max','Ponto','Dentro Elips','Nº pontos analisados']
ncasos=1000
seed=74
pont=[]
res=[]
...
for j in range (ncasos):
    ...
    df = pd.DataFrame(list(zip(ai,aii,aiii,aiiii,ci,cii,ciii,ciiii,tim,res,pont,dentro,totalpontos)), columns=columns)
    df.to_excel(r"C:\Users\lucas_mzj9txp\Desktop\AL4D.xlsx")